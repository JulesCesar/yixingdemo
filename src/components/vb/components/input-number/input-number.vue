...
<script lang="babel">
    ...
    export default {
        ...
        watch: {
            value(val) {
                // console.log('this.relValue ',this.relValue);
                this.relValue = val;
            },
            relValue(val) {
                // if (this.relValue === this.value) return;
                // console.log('relValue ',val);
        ...
        methods: {
            handleInput(event) {
                ...
                if (curValue.indexOf('.')>0) {
                    let relSplitValues = curValue.split('.');
                    curValue = parseInt(relSplitValues[0]) + '.' 
                    if (relSplitValues.length>1) {
                        curValue += relSplitValues[1]
                    }
                } else {
                    curValue = curValue === '' ? curValue : parseInt(curValue)+'';
                }
            ...
            setValue(value) {
                // console.log("DB",value,this.relValue)
